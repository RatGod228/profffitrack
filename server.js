...
const sendEmail = async (to, subject, text) => {
  ...
  return new Promise((resolve) => {
    // Use ASCII only in subject to avoid UTF8 issues
    const asciiSubject = 'Password Reset Code - ProfitTrack';
    const emailData = {
      personalizations: [{
        to: [{ email: to }]
      }],
  ...
  'POST /api/auth/login': async (req, res) => {
    try {
      ...
      if (!user) {
        ...
        res.writeHead(401); 
        return res.end(JSON.stringify({ error: 'Неверный логин/email или пароль' }));
      }
      const hashedInput = hashPassword(password);
      console.log('Stored hash:', user.password.substring(0, 20) + '...');
      console.log('Input hash:', hashedInput.substring(0, 20) + '...');
  ...
  'POST /api/auth/forgot-password': async (req, res) => {
    try {
      ...
      if (emailResult.sent) {
        ...
        res.writeHead(500); 
        res.end(JSON.stringify({ error: 'Не удалось отправить email. Попробуйте позже.' }));
      }
    } catch (e) {
